...
import viso.app.TransactionException;
...
import viso.impl.framework.service.data.store.DataStoreException;
...
import viso.test.framework.util.DummyProfileCoordinator;
...
public class TestDataStoreImpl extends Assert {
	...
	public TestDataStoreImpl() {
		this(defaultEnv == null ? defaultEnv = new BasicDataStoreTestEnv(System
				.getProperties()) : defaultEnv);
	}
	...
	public void testGetClassInfoAfterRestart() throws Exception {
		...
		store = createDataStore();
		txn = createTransaction(UsePrepareAndCommit.ARBITRARY);
		assertEquals(Arrays.toString(bytes), Arrays.toString(store
				.getClassInfo(txn, id)));
	}
	...
	protected DataStore createDataStore(Properties props) throws Exception {
		DataStore store = new DataStoreProfileProducer(new DataStoreImpl(props,
				systemRegistry, txnProxy), DummyProfileCoordinator
				.getCollector());
