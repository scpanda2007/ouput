...
import viso.app.ManagedObjectRemoval;
import viso.app.ManagedReference;
import viso.app.NameNotBoundException;
...
import viso.framework.service.DataService;
...
import viso.impl.framework.service.data.store.DataStoreProfileProducer;
import viso.impl.framework.service.store.net.DataStoreClient;
import viso.impl.util.AbstractKernelRunnable;
import viso.impl.util.TransactionContextFactory;
import viso.impl.util.TransactionContextMap;
...
public final class DataServiceImpl implements DataService {
	...
	private static final String CLASSNAME = "viso.impl.framework.service.data.DataServiceImpl";
	...
	private static final LoggerWrapper abortLogger = new LoggerWrapper(Logger
			.getLogger(CLASSNAME + ".abort"));
	...
	private ManagedObject getBindingInternal(String name,
		...
		try {
			...
			try {
				result = context.getBinding(getInternalName(name,
						serviceBinding), forUpdate);
			} catch (NameNotBoundException e) {
	...
	private String nextBoundNameInternal(String name, boolean serviceBinding) {
		...
		try {
			context = getContext();
			String result = getExternalName(context
					.nextBoundName(getInternalName(name, serviceBinding)),
					serviceBinding);
			if (logger.isLoggable(Level.FINEST)) {
				logger.log(Level.FINEST,
