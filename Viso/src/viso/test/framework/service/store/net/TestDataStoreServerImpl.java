package viso.test.framework.service.store.net;
...
import viso.impl.framework.service.store.net.DataStoreServerImpl;
...
public class TestDataStoreServerImpl extends TestCase {
	...
	private static final String DataStoreNetPackage = "viso.framework.impl.service.data.store.net";
	...
	public void testReaperConcurrency() throws Exception {
		...
		tid = -1;
		tearDown();
		props.setProperty("viso.framework.txn.timeout", "2");
	...
	private void testConcurrent(final Runnable action) throws Exception {
		...
		assertTrue("Blocking thread did not start", flag.tryAcquire(100,
		...
		try {
			...
			/* Clean up */
			server.abort(tid2);
			assertTrue("Blocking thread didn't complete", flag.tryAcquire(100,
					TimeUnit.MILLISECONDS));
			if (aborted.get()) {
				tid = -1;
			}
