...
import viso.app.TransactionConflictException;
import viso.app.TransactionTimeoutException;
...
import viso.framework.service.store.db.DbDatabase;
 ...
 * "http://www.oracle.com/database/berkeley-db/db/index.html">Berkeley DB</a>.
 * <p>
 * 
 ...
 * 
 * <dd style="padding-top: .5em">The interval in milliseconds between checkpoint
 * operations that flush changes from the database log to the database.
 * <p>
 * 
 ...
 * The value must be greater than <code>0</code>, and should be less than the
 * transaction timeout.
 * <p>
 * 
 ...
 * <code>com.sun.sgs.impl.service.data.db.bdb</code> to log information at the
...
public class BdbEnvironment implements DbEnvironment {
	 ...
	 */
	public static final String CACHE_SIZE_PROPERTY = PACKAGE + ".cache.size";
	 ...
	 */
	public static final String CHECKPOINT_INTERVAL_PROPERTY = PACKAGE
			+ ".checkpoint.interval";
	 ...
	 */
	public static final String CHECKPOINT_SIZE_PROPERTY = PACKAGE
			+ ".checkpoint.size";
	 ...
	 */
	public static final String REMOVE_LOGS_PROPERTY = PACKAGE + ".remove.logs";
	 ...
	 */
	public BdbEnvironment(String directory, Properties properties,
		...
		long cacheSize = wrappedProps.getLongProperty(CACHE_SIZE_PROPERTY,
				DEFAULT_CACHE_SIZE, MIN_CACHE_SIZE, Long.MAX_VALUE);
	 ...
	 * during an operation. Throws an Error if recovery is needed. Only converts
	 * Berkeley DB transaction exceptions to the associated exceptions if
	 * convertTxnExceptions is true.
	 */
	static RuntimeException convertException(DatabaseException e,
			boolean convertTxnExceptions) {
		/*
		 * Don't include DatabaseExceptions as the cause because, even though
		 * that class implements Serializable, the Environment object optionally
		 * contained within them is not. -tjb@sun.com (01/19/2007)
		 */
		if (convertTxnExceptions && e instanceof LockNotGrantedException) {
			...
			throw new DbDatabaseException("Unexpected database exception: " + e);
		}
	}
	...
	public DbDatabase openDatabase(DbTransaction txn, String fileName,
		...
		return new BdbDatabase(env, BdbTransaction.getBdbTxn(txn), fileName,
				create);
	}
