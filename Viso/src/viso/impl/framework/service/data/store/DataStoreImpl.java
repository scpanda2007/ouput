...
import viso.app.ObjectNotFoundException;
...
import viso.framework.service.store.ClassInfoNotFoundException;
...
import viso.impl.framework.service.data.store.DbUtilities.Databases;
import viso.util.tools.LoggerWrapper;
import viso.util.tools.PropertiesWrapper;

import static viso.impl.framework.service.data.store.DataStoreHeader.PLACEHOLDER_OBJ_VALUE;
import static viso.impl.framework.service.data.store.DataStoreHeader.FIRST_PLACEHOLDER_ID_KEY;
import static viso.impl.framework.service.data.store.DataStoreHeader.ALLOCATION_BLOCK_SIZE;
import static viso.impl.framework.service.data.store.DataStoreHeader.QUOTE_OBJ_VALUE;
...
public class DataStoreImpl extends AbstractDataStore {
	...
	private static final String CLASSNAME = "viso.impl.framework.service.data.store.DataStoreImpl";
	...
	public static final String ENVIRONMENT_CLASS_PROPERTY = "viso.impl.framework.service.data.store.db.environment.class";

	/** The default environment class. */
	public static final String DEFAULT_ENVIRONMENT_CLASS = "viso.impl.framework.service.data.store.db.bdb.BdbEnvironment";
	...
	protected long createObjectInternal(Transaction txn) {
		...
		if (useAllocationBlockPlaceholders && result == objectIdInfo.first()) {
			 ...
			 */
			oidsDb.put(txnInfo.dbTxn, DataEncoding.encodeLong(objectIdInfo
					.last()), PLACEHOLDER_DATA);
		}
		return result;
	}
	...
	protected BindingValue getBindingInternal(Transaction txn, String name) {
		TxnInfo txnInfo = checkTxn(txn);
		byte[] value = namesDb.get(txnInfo.dbTxn, DataEncoding
				.encodeString(name), false);
		if (value == null) {
	...
	protected BindingValue removeBindingInternal(Transaction txn, String name) {
		TxnInfo txnInfo = checkTxn(txn);
		boolean found = namesDb.delete(txnInfo.dbTxn, DataEncoding
				.encodeString(name));
		if (found) {
			txnInfo.modified = true;
			return new BindingValue(1, txnInfo.nextName(name, namesDb));
	...
	protected int getClassIdInternal(Transaction txn, byte[] classInfo) {
		checkTxn(txn);
		return DbUtilities.getClassId(env, classesDb, classInfo, txn
				.getTimeout());
	}
	...
	protected byte[] getClassInfoInternal(Transaction txn, int classId)
		...
		checkTxn(txn);
		byte[] result = DbUtilities.getClassInfo(env, classesDb, classId, txn
				.getTimeout());
		if (result != null) {
			return result;
		} else {
